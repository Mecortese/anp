...
const fmtChange = (p: number, entry: number, type: 'long' | 'short') => {
  const pct = type === 'long' ? ((p - entry) / entry) * 100 : ((entry - p) / entry) * 100;
  return { text: (pct >= 0 ? '+' : '') + pct.toFixed(2) + '%', positive: pct >= 0 };
};

export function SetupCard({ setup }: { setup: TradeSetup; leverage?: 1 | 2 | 3 | 5 }) {
  const [currentPrice, setCurrentPrice] = useState<number | null>(null);
  const [taken, setTaken] = useState(setup.taken || false);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    const okxSym = setup.symbol.includes('-') ? setup.symbol : `${setup.symbol.slice(0, -4)}-USDT`;
    const path = `/api/v5/market/ticker?instId=${okxSym}`;
    const req = fetch(`https://www.okx.com${path}`).then(r => r.json()).catch(() => null);
    req.then((data: any) => {
      if (data?.data?.[0]?.last) setCurrentPrice(parseFloat(data.data[0].last));
    });
  }, [setup.symbol]);
  ...
  let livePnl = null, liveStatus: 'open' | 'won' | 'lost' | 'breakeven' | null = null;
  if (currentPrice !== null) {
    const pct = setup.type === 'long'
      ? ((currentPrice - setup.entryPrice) / setup.entryPrice) * 100
      : ((setup.entryPrice - currentPrice) / setup.entryPrice) * 100;
    livePnl = pct;
    if (setup.type === 'long') {
      if (currentPrice <= setup.stopLoss) liveStatus = 'lost';
      else if (currentPrice >= setup.takeProfit) liveStatus = 'won';
      else liveStatus = 'breakeven';
    } else {
      if (currentPrice >= setup.stopLoss) liveStatus = 'lost';
      else if (currentPrice <= setup.takeProfit) liveStatus = 'won';
      else liveStatus = 'breakeven';
  ...
  return (
    <div className={`bg-gray-900/80 backdrop-blur rounded-xl p-4 border border-gray-800/50 border-l-4 ${riskColor} ${taken ? 'opacity-60' : 'hover:border-gray-700/80 transition-colors'}`}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-2">
        ...
        <div className="bg-gray-950/50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-600 uppercase mb-0.5">Actual</div>
          {currentPrice ? (
            <div className={`font-mono font-bold text-sm ${liveStatus === 'won' ? 'text-emerald-400' : liveStatus === 'lost' ? 'text-red-400' : 'text-white'}`}>
              {fmtPrice(currentPrice)}
            </div>
          ) : (
            <div className="font-mono text-gray-600 text-sm">...</div>
          )}
        </div>
        <div className="bg-gray-950/50 rounded-lg p-2 text-center">
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        ...
        <div className="bg-gray-950/50 rounded-lg p-2 text-center">
          <div className="text-[10px] text-gray-600 uppercase mb-0.5">P&L Live</div>
          {livePnl !== null ? (
            <div className={`font-black text-sm ${livePnl >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
      ...
      <div className="grid grid-cols-4 gap-2 text-[10px] text-gray-600 mb-3">
        ...
        <div>Lev: <span className={levColor}>{setup.leverageRecommendation}x</span></div>
      </div>
